...
public class AplicacionWebTest {
	...
	void cuandoSeAgregaUnaMuestraSeNotificaALasZonaQueLaContienen() {
		...
		app.zonasDeLaMuestra(muestraMock1)
				.forEach(zona -> verify(zona).notificar(muestraMock1, Aspecto.MUESTRA_ENVIADA));
	}
	...
	void puedoObtenerLaCantidadDeMuestrasEnviadasPorUnUsuarioEnUnaListaDeMuestras() {
		...
		Arrays.asList(muestraMock1, muestraMock3).forEach(muestra -> verify(muestra).fueEnviadaPor(userMock));
	...
	void puedoObtenerLaCantidadDeOpinionesPorUnUsuarioEnUnaListaDeMuestras() {
		...
		Arrays.asList(muestraMock1, muestraMock3).forEach(muestra -> verify(muestra).elUsuarioYaOpino(userMock));
